...
public class IdeeDaoImpl implements IdeeDao{
	...
	public List<Object[]> getIdeesListSortByTopUserWriters(){	    
    	List<Object[]> idees = em.createQuery("select i.user.id , count(i.user.id) AS cnt from Idee i GROUP by i.user.id order by cnt desc", Object[].class)
    			.setMaxResults(3)
    			.getResultList();	   
	    return idees;
	}
