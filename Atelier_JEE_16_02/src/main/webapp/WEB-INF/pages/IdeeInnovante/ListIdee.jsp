   ...
   <tbody>   
    <c:forEach items="${IdeesList}" var="i" varStatus="loop">   
	    <tr>
	        ...
	        <td><img style="max-width: 200px; max-height:200px" src="${i.image}"/></td>
	        <td>
	        <!-- Ici débute notre grousse boucle d'affichage des votes  -->
	        <!-- Premier contrôle si notre liste de vote est null  -->
	        <c:if test="${empty i.vote }">
	        							<!-- Un utilisateur ne peut pas voter pour sa propre idée + controle date moins 7 jours après création de l'idée -->
	        							<!-- Sinon affichage des boutons top et flop pour voter : otherwise -->
										<c:choose>												
			...
			<!-- notre idée a bien des votes enregistrés on fait nos contrôle -->							    
			<c:if test="${!empty i.vote }">
									<!-- L'utilisateur a déjà voté pour cette idée : affichage du set -->	
									...
									<!-- Notre utilisateur n'a pas voté pour cette question :  -->										 
				...
				<!-- Variable repasse à false (utilisateur a voté?) -->
